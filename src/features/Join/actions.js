...
export function register(user) {
  return dispatch => {
    ...
    return post(url, mappedUser).then(
      () => {
...
export function validateEmail(email) {
  return dispatch => {
    ...
    return get(`${config.api.endpoint}users/check/email/${email}`).then(
      () => {
...
export function validateAlias(alias) {
  return dispatch => {
    ...
    return get(`${config.api.endpoint}users/check/alias/${alias}`).then(
      () => {
...
export function validatePasswords(password1, password2) {
  ...
  if (!isStrong) {
    errors.push('Needs 8 or more characters with at least one being uppercase.');
  }
