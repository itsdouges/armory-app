...
const Specialization = ({ data, traits, specializations }) => {
  ...
  return (
    <div className={styles.root}>
      ...
      <Trait className={styles.minorTraitColumn} data={getTrait(traits, spec.minor_traits[0])} />

      <div className={styles.majorTraitColumn}>
        {spec.major_traits.slice(0, 3).map((id) =>
          <Trait key={id} data={getTrait(traits, id)} />)}
      </div>

      <Trait className={styles.minorTraitColumn} data={getTrait(traits, spec.minor_traits[1])} />
      ...
      <Trait className={styles.minorTraitColumn} data={getTrait(traits, spec.minor_traits[2])} />
